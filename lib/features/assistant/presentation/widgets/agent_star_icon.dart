...
class _AgentStarPainter extends CustomPainter {
  ...
  final Color bodyStrokeColor = const Color(0xFFE5E7EB); // Gray 200 (Whitish-gray) for Body
  final Color faceColor = Colors.white; // Make face pure white
  final Color strokeColor = const Color(0xFFE0E0E0); // Grayish white for border (was Colors.white)
  ...
  void _drawStar(Canvas canvas, double cx, double cy, double outerRadius, double scale) {
    ...
    final paintStroke = Paint()
      ..color = (isHollow || isWhiteFilled) ? bodyStrokeColor : ((mode == AgentStarMode.dashboard) ? strokeColor : Colors.white)
  ...
  void _drawFace(Canvas canvas, double cx, double cy, double radius, double scale) {
    ...
    final Color featureColor = isWhiteFilled ? darkBgColor : (isHollow ? Colors.white : faceColor);

    final paintStroke = Paint()
      ..color = featureColor
      ..style = PaintingStyle.stroke
      ..strokeWidth = strokeWidthBase * elementScale // Bold stroke for mouth/brows
      ..strokeCap = StrokeCap.round;
    ...
    void drawClosedEye(double bx, double by) {
         final w = 7.0 * ratio;
         final p = Path()..moveTo(bx - w, by)..lineTo(bx + w, by);
    ...
    void drawFlatEye(double bx, double by) {
        final w = 7.0 * ratio;
        final h = 3.0 * ratio;
        final p = Path()..moveTo(bx - w, by - h)..lineTo(bx + w, by - h);
        canvas.drawPath(p, paintStroke);
    }
    ...
    if (isStatic) {
       ...
       final double staticEyeRadius = 3.5 * ratio * (elementScale > 1.0 ? 1.5 : 1.0); // Slightly less aggressive scale for radius than stroke
       canvas.drawCircle(Offset(cx - eyeOffset, eyeY), staticEyeRadius, paintFill);
       canvas.drawCircle(Offset(cx + eyeOffset, eyeY), staticEyeRadius, paintFill);
    ...
    if (emotionState == 'task_neutral') {
        browPath.moveTo(x - eyeOffset - bW, browY); browPath.lineTo(x - eyeOffset + bW, browY);
        browPath.moveTo(x + eyeOffset - bW, browY); browPath.lineTo(x + eyeOffset + bW, browY);
    } else if (emotionState == 'task_wink') {
        browPath.moveTo(x - eyeOffset - bW, browY + (1*ratio));
        browPath.lineTo(x - eyeOffset + bW, browY - (2*ratio));
        browPath.moveTo(x + eyeOffset - bW, browY); browPath.lineTo(x + eyeOffset + bW, browY);
    } else if (emotionState == 'task_smile' || emotionState == 'happy' || emotionState == 'smile') {
        browPath.moveTo(x - eyeOffset - bW7, browY + (2*ratio));
        browPath.quadraticBezierTo(x - eyeOffset, browY - (5*ratio), x - eyeOffset + bW7, browY + (2*ratio));
        
        browPath.moveTo(x + eyeOffset - bW7, browY + (2*ratio));
        browPath.quadraticBezierTo(x + eyeOffset, browY - (5*ratio), x + eyeOffset + bW7, browY + (2*ratio));
    } else if (emotionState == 'reading_loop') {
        browPath.moveTo(x - eyeOffset - bW, browY - (1*ratio)); browPath.lineTo(x - eyeOffset + bW, browY - (1*ratio));
        browPath.moveTo(x + eyeOffset - bW, browY - (1*ratio)); browPath.lineTo(x + eyeOffset + bW, browY - (1*ratio));
    } else if (emotionState == 'skeptical') {
        browPath.moveTo(x - eyeOffset - bW, browY); browPath.lineTo(x - eyeOffset + bW, browY);
        final r = 6.0 * ratio;
        final rect = Rect.fromCircle(center: Offset(x + eyeOffset, browY), radius: r);
    ...
    switch(emotionState) {
        ...
        case 'task_smile':
          ...
          break;
        case 'blink':
           drawClosedEye(x - eyeOffset, eyeY); drawClosedEye(x + eyeOffset, eyeY);
           break;
        case 'winkLeft':
           drawClosedEye(x - eyeOffset, eyeY); drawOpenEye(x + eyeOffset, eyeY);
           break;
        case 'winkRight':
           drawOpenEye(x - eyeOffset, eyeY); drawClosedEye(x + eyeOffset, eyeY);
           break;
        case 'happy':
           drawHappyEye(x - eyeOffset, eyeY); drawHappyEye(x + eyeOffset, eyeY);
           break;
        case 'smile':
           drawOpenEye(x - eyeOffset, eyeY); drawOpenEye(x + eyeOffset, eyeY);
           break;
         case 'skeptical':
           drawFlatEye(x - eyeOffset, eyeY); drawOpenEye(x + eyeOffset, eyeY);
           break;
         case 'reading_loop':
            ...
            double lookFactor = 0; 
            if (t < 3) { lookFactor = 0; } 
            else if (t < 4) { final phase = t - 3; lookFactor = phase * phase * (3 - 2 * phase); }
            else if (t < 10) { lookFactor = 1; }
            else if (t < 11) { final phase = t - 10; final smooth = phase * phase * (3 - 2 * phase); lookFactor = 1 - smooth; }
            else { lookFactor = 0; }
    ...
    if (emotionState == 'task_wink') {
      final rect = Rect.fromCircle(center: Offset(x, mouthY - (1*ratio)), radius: 9*ratio);
      mouthPath.addArc(rect, 0.15, (math.pi - 0.15) - 0.15);
    } else if (emotionState == 'task_smile') {
       final rect = Rect.fromCircle(center: Offset(x, mouthY - (2*ratio)), radius: 10*ratio);
       mouthPath.addArc(rect, 0.1, (math.pi - 0.1) - 0.1);
    } else if (emotionState == 'task_neutral') {
       mouthPath.moveTo(x - (5*ratio), mouthY);
       mouthPath.quadraticBezierTo(x, mouthY + (1*ratio), x + (5*ratio), mouthY);
    } else if (emotionState == 'reading_loop') {
       final rect = Rect.fromCircle(center: Offset(x, mouthY - (1*ratio)), radius: 8*ratio);
       ...
       mouthPath.moveTo(x + (5*ratio), mouthY);
       mouthPath.quadraticBezierTo(x, mouthY + (2*ratio), x - (7*ratio), mouthY - (3*ratio));
    } else if (emotionState == 'winkRight') {
       mouthPath.moveTo(x - (5*ratio), mouthY);
       mouthPath.quadraticBezierTo(x, mouthY + (2*ratio), x + (7*ratio), mouthY - (3*ratio));
    } else if (emotionState == 'skeptical') {
       mouthPath.moveTo(x - (4*ratio), mouthY); mouthPath.lineTo(x + (4*ratio), mouthY);
    } else {
       mouthPath.moveTo(x - (5*ratio), mouthY);
       mouthPath.quadraticBezierTo(x, mouthY + (1*ratio), x + (5*ratio), mouthY);
    }
