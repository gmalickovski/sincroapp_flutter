...
class _FocoDoDiaScreenState extends State<FocoDoDiaScreen> {
  ...
  List<TaskModel> _filterTasks(List<TaskModel> rawTasks, int? userPersonalDay) {
    // 0. Pre-process Flow tasks dynamically
    final now = DateTime.now();
  ...
  void _onTaskToggled(TaskModel task, bool isCompleted) async {
    if (_isSelectionMode) return;

    if (task.recurrenceCategory == 'flow') {
      if (isCompleted) {
        // Complete a virtual flow instance
        final newTaskId = const Uuid().v4();
        final instanceTask = task.copyWith(
          id: newTaskId,
          completed: true,
          completedAt: DateTime.now().toUtc(),
          // keep dueDate as it was generated by the filter (the specific day)
          recurrenceCategory: 'flow_instance',
          recurrenceId: task.id,
        );
        await _supabaseService.addTask(_userId, instanceTask);
      }
    } else if (task.recurrenceCategory == 'flow_instance') {
      if (!isCompleted) {
        // Un-complete a flow instance -> delete it entirely to revert to virtual task
        await _supabaseService.deleteTask(_userId, task.id);
      } else {
        // If it was somehow incomplete and marked complete (shouldn't happen with flow_instance, but just in case)
