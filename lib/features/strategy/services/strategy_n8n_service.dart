...
class StrategyN8NService {
  ...
  static Future<String> analyzeProfessionCompatibility({
    ...
    if (webhookUrl == null || webhookUrl.isEmpty) {
      webhookUrl =
          'https://n8n.studiomlk.com.br/webhook/sincroapp-professional-aptitude';
      debugPrint(
          '⚠️ .env not loaded. Using Hardcoded Professional Aptitude Webhook.');
    }
    ...
    final expressionReduced =
        NumerologyEngine.reduceNumber(expressionRaw, mestre: true);

    // 2. Lookup Ground Truth Content
    // Try exact match (e.g. 11, 22). If not found (e.g. 33), reduce to single digit (6).
    VibrationContent? aptitudeContent =
        ContentData.textosAptidoesProfissionais[expressionReduced];
    if (aptitudeContent == null) {
      final singleDigit =
          NumerologyEngine.reduceNumber(expressionRaw, mestre: false);
    ...
    final payload = {
      ...
      'user': {
        ...
        'numerology': {
          ...
          'destiny': NumerologyEngine.reduceNumber(
              profile.numeros['destino'] ?? 0,
              mestre: true),
          'path': NumerologyEngine.reduceNumber(profile.numeros['missao'] ?? 0,
              mestre: true),
        }
      },
...
''',
    ...
    try {
      debugPrint(
          '🚀 Sending Professional Aptitude Request to N8N: $webhookUrl');
