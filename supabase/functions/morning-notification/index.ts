// supabase/functions/morning-notification/index.ts
// Edge Function: Sends morning motivation push to all users (08:30)
// Called by pg_cron daily

import { serve } from "https://deno.land/std@0.168.0/http/server.ts"
import { createClient } from "https://esm.sh/@supabase/supabase-js@2"

// Reusable Firebase Auth (same pattern as send-task-reminders)
...
serve(async (_req: Request) => {
    try {
        ...
        // Group tokens by user (avoid duplicate pushes)
        ...
        for (const [_userId, userTokens] of Object.entries(tokensByUser)) {
                        ...
                        `https://fcm.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/messages:send`,
                        {
                            ...
                            body: JSON.stringify({
                                message: {
                                    ...
                                    notification: {
                                        title: 'Bom dia! ☀️',
                                        body: 'Confira como as energias de hoje podem te guiar e veja suas tarefas para o dia.',
                                    },
        ...
        console.log(`[MORNING] Sent to ${sentCount} tokens across ${Object.keys(tokensByUser).length} users`)

        return new Response(
            JSON.stringify({ sent: sentCount, users: Object.keys(tokensByUser).length }),
            { headers: { 'Content-Type': 'application/json' } }
        )
    } catch (err) {
        console.error('[MORNING] Fatal:', err)
