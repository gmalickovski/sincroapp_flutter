...
server {
    ...
    # Servir arquivos reais a partir da raiz do build (strip do prefixo /app/)
    # Suporta trailing slash no final da URL (ex.: flutter_bootstrap.js/)
    ...
    # Mapear manifest e service worker de /app/ para a raiz
    ...
    # Mapear diretórios de assets e ícones quando solicitados sob /app/
    location ^~ /app/assets/ {
        alias /var/www/webapp/sincroapp_flutter/build/web/assets/;
        expires 1y;
        add_header Cache-Control "public, immutable";
    }
    location ^~ /app/icons/ {
        alias /var/www/webapp/sincroapp_flutter/build/web/icons/;
        expires 1y;
        add_header Cache-Control "public, immutable";
    }
    location = /app/favicon.png {
    ...
    add_header Content-Security-Policy "default-src 'self' https:; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.tailwindcss.com https://www.gstatic.com https://unpkg.com https://www.google.com https://accounts.google.com https://recaptchaenterprise.googleapis.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://unpkg.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' https://firestore.googleapis.com https://*.cloudfunctions.net wss://*.firebaseio.com https://identitytoolkit.googleapis.com https://recaptchaenterprise.googleapis.com https://www.gstatic.com https://www.google.com https://content-firebaseappcheck.googleapis.com https://accounts.google.com https://fonts.gstatic.com; frame-src https://www.google.com https://accounts.google.com; worker-src 'self' blob: https://www.google.com;" always;
